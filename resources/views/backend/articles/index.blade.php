...
@section('content')
			...
			<div class="card mb-4">
				<div class="card-header pb-0">
					...
					@if(session('success'))
						...
						<div class="mt-2">
							<img src="{{ asset('images/'.session('image')) }}" alt="Uploaded Image" width="300">
						</div>
					@endif
				</div>
				<div class="card-body p-4 pt-2">
					<button onclick="modalCreate(`{{ route('articles.store') }}`)" type="button" class="btn bg-gradient-success mb-3" data-bs-toggle="modal" data-bs-target="#modal-form">Tambah baru</button>
						...
						<table class="table align-items-center mb-0 display" id="table">
							<thead>
								<tr>
									...
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">Tautan</th>
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">Gambar</th>
									<th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-8 p-1">Kategori</th>
	...
	@includeIf('backend.articles.modal-form')
@endsection

@push('scripts')
<script>
	let table = new DataTable('#table', {
		ajax: '{{ route('articles.api') }}',
		stateSave: true,
		processing: true,
		autoWidth: false,
		serverSide: true,
		columns: [
			...
			},
			{
				...
				render: function (data, type, row) {
					if (data) {
							const maxLength = 20;
							const urlLength = data.length;

							if (urlLength > maxLength) {
									const start = data.substring(0, 7);
									const end = data.substring(urlLength - 7);
									return `${start}...${end}`;
							} else {
									return data;
							}
