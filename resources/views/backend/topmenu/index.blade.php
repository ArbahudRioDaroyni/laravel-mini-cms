...
@section('content')
		...
		<div class="col-12">
			...
			<div class="card mb-4">
				...
				<div class="card-body p-4 pt-2">
					<button onclick="modalCreate(`{{ route('topmenu.store') }}`)" type="button" class="btn bg-gradient-success mb-3" data-bs-toggle="modal" data-bs-target="#modal-form">Tambah baru</button>
						...
						<table class="table align-items-center mb-0 display" id="table">
							<thead>
								<tr>
									...
									<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">Menu</th>
									<th class="text-left text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">Induk</th>
									<th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">URL</th>
									<th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-8">Position</th>
	...
	@includeIf('backend.topmenu.modal-form')
@endsection

@push('scripts')
<script>
	let table = new DataTable('#table', {
		ajax: '{{ route('topmenu.api') }}',
		stateSave: true,
		processing: true,
		autoWidth: false,
		serverSide: true,
